...
LOCAL_IP = "10.26.84.107"  # <-- change this to your computer's IP
FLASK_PORT = 5000

# Path to podcasts folder
PODCAST_FOLDER = os.path.join(os.path.dirname(__file__), "app", "static", "podcasts")

# Rich podcast content with transcripts
PODCAST_METADATA = [
    {
...
def seed_podcasts():
    """Seed podcasts from local files with rich metadata"""
    # Check if podcasts already exist
    existing_podcasts = EducationalContent.query.filter_by(category='podcasts').count()
    if existing_podcasts > 0:
        print(f"Podcasts already exist ({existing_podcasts} items). Skipping seed.")
        return
    
    if not os.path.exists(PODCAST_FOLDER):
        print(f"Folder not found: {PODCAST_FOLDER}")
        print("Creating podcasts with external URLs instead...")
        
        # Use external URLs if local folder doesn't exist
        for i, metadata in enumerate(PODCAST_METADATA):
            new_content = EducationalContent(
                title=metadata['title'],
                subtitle=metadata.get('subtitle'),
                content_type="podcast",
                content_url=f"https://www.soundhelix.com/examples/mp3/SoundHelix-Song-{i+1}.mp3",
        ...
        db.session.commit()
        print(f"✅ Successfully seeded {len(PODCAST_METADATA)} podcasts with external URLs!")
        return

    # If local folder exists, use local files
    files = [f for f in os.listdir(PODCAST_FOLDER) if f.endswith(".mp3")]
    if not files:
        print("No mp3 files found in the podcasts folder.")
        return

    # Match files with metadata
    for i, file_name in enumerate(sorted(files)):
        content_url = f"http://{LOCAL_IP}:{FLASK_PORT}/static/podcasts/{file_name}"
        
        # Use metadata if available, otherwise use filename
        if i < len(PODCAST_METADATA):
            metadata = PODCAST_METADATA[i]
            title = metadata['title']
            subtitle = metadata.get('subtitle')
            content_text = metadata['content_text']
            reading_time = metadata['reading_time']
    ...
    db.session.commit()
    print(f"✅ Successfully seeded {len(files)} podcasts from local files!")
